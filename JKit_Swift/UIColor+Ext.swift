...
extension UIColor {
    ...
    public static func j_color( hexString: String, alpha: CGFloat = 1.0 ) -> UIColor {
        ...
        if cString.hasPrefix("0X") || cString.hasPrefix("0x") {
            ...
            cString = String(cString[cString.index(cString.startIndex, offsetBy: 2)...]) // = str.substring(from: index) In Swif 3
        }
        
        if cString.hasPrefix("#") {
            
            cString = String(cString[cString.index(cString.startIndex, offsetBy: 1)...]) // = str.substring(from: index) In Swif 3
        }
